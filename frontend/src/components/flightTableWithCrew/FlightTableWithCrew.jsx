      ...
      async function fetchFlightsWithCrew(){
        try {
          ...
          let refinedData = data.data.flights.map(flight => {
            ...
            let checkCaptain = flight.crewMembers.some(role => 'captain' in role);
            let checkCoPilot = flight.crewMembers.some(role => 'copilot' in role);
            let checkFlightAttendant = flight.crewMembers.some(role => 'flightAttendant' in role);
            let checkCrew = checkCaptain && checkCoPilot && checkFlightAttendant;
          ...
          if(activeTab === "crew not complete"){
            const filteredData = refinedData.filter(
              (flight) => flight.status === "no captain" || flight.status === "no co-pilot" || flight.status === "no cabin crew"
            )
            setFlights(filteredData)
          }
